...
class AdminRenderer(BrowsableAPIRenderer):
    ...
    def render(self, data, accepted_media_type=None, renderer_context=None):
        ...
        if response.status_code == status.HTTP_201_CREATED and 'Quiz' in response:
            response.status_code = status.HTTP_303_SEE_OTHER
            response['Quiz'] = request.build_absolute_uri()
            ret = ''
        ...
        if response.status_code == status.HTTP_204_NO_CONTENT:
            ...
            try:
                ...
                response['Quiz'] = self.get_breadcrumbs(request)[-2][1]
            except KeyError:
                # Otherwise reload current URL to get a 'Not Found' page.
                response['Quiz'] = request.full_path
            ret = ''
