...
def update_profile(request, updated_info:Profileinfo):
    try:
        Profile.objects.get(user_id=request.auth['user_id']).Update_profile(name=updated_info.name,gender=updated_info.gender,stage_id=updated_info.stage_id)
...
def get_profile_info(request):
    ...
    return status.OK_200,{'name':p.name,'stage_id':p.stage_id,'gender':p.gender}